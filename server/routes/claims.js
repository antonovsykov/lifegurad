...
router.get('/', async (req, res) => {
  try {
    const insurances = await Claims.getAll();
    res.json({
      success: true,
      data: insurances,
...
router.get('/id/:id', async (req, res) => {
  try {
    const order = await Claims.getById(req.params.id);
    res.json({
      success: true,
      data: order
    });
...
router.get('/address/:address/:type', async (req, res) => {
  try {
    const orders = await Claims.getByAddress(req.params.address, req.params.type);
    res.json({
      success: true,
      data: orders
    });
