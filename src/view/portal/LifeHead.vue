    ...
    <div class="wrap">
      ...
      <div :class="isMobile ? 'right-side-col' : 'right-side'">
        ...
        <div class="wallet-connect" id="wallet-app">
          ...
          <button class="wallet-info wallet-target" v-else @click.stop="showWalletInfo">
            <img :src="walletInfo.walletInfo?.icon" alt="Lifeguard Logo" />
            <span v-if="!isMobile">{{ formattedAddress }}</span>
          </button>
          <!-- 提示框（条件渲染） -->
          <div class="tooltip" ref="tooltipRef" v-if="showTooltip" :style="{
            ...
            <button class="claim-btn" @click="toAccount">
              <svg t="1756422636950" class="icon" viewBox="0 0 1259 1024" version="1.1"
                xmlns="http://www.w3.org/2000/svg" p-id="19365" width="20" height="20" fill="currentColor">
                <path
            ...
            <button class="claim-btn" @click="toPolicy">
              <svg t="1756423106047" class="icon" viewBox="0 0 1137 1024" version="1.1"
                xmlns="http://www.w3.org/2000/svg" p-id="27907" width="20" height="20" fill="currentColor">
                <path
            ...
            <button class="claim-btn" @click="toApply">
              <svg t="1756423312552" class="icon" viewBox="0 0 1024 1024" version="1.1"
                xmlns="http://www.w3.org/2000/svg" p-id="33137" width="20" height="20" fill="currentColor">
                <path
...
const handleClickOutside = (e) => {
  // 若 tooltip 已显示，且点击位置不在 tooltip 内部
  if (showTooltip.value && tooltipRef.value) {
    // 判断点击目标是否是 tooltip 本身或其后代元素
    const isClickInside = tooltipRef.value.contains(e.target);
    if (!isClickInside) {
      // 点击外部：关闭 tooltip
      showTooltip.value = false;
    }
